 ...
 * hand-decoding the raw PGRST sentinel. `code` is one of: `AX_SESSION_REVOKED`,
 * `AX_APP_LOCKED` (real, previously-authenticated account — show the PIN unlock overlay),
 * or `AX_APP_LOCKED_NO_ACCOUNT` (a session with no profile yet — never happens in practice
 * today since a brand-new session gets a full idle window before it can lock, but clients
 * must not assume; do not show the PIN overlay for this code). This account-exists
 * distinction is encoded directly in `code` rather than as a separate body field or
 * response header: PostgREST's PGRST convention only extracts code/message/details/hint
 * from the raised message JSON onto the response body, silently dropping any other key
 * (verified live); a custom response header doesn't work for browser clients either, since
 * PostgREST's Access-Control-Expose-Headers is a fixed built-in list a custom header can't
 * join — a browser's fetch() can never read it even though it's present in the raw HTTP
 * response (verified live: curl sees it, a real browser fetch() cannot). code/message are
 * the two fields PostgREST reliably delivers to every client, verified live repeatedly —
 * encode everything a client needs into those.
 */
